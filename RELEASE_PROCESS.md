...
-   **Automation:** The workflow handles everything automatically:
    ...
    -   **Renders the store screenshots** *(internal releases)*: `:marketing-screenshots:updateMarketingScreenshots` runs on the tag and attaches the Play set as `store-listing-screenshots-<versionCode>.zip` and the five desktop PNGs to the release. A failed render attaches the committed set instead, so the metainfo's screenshot URLs still resolve, and the job summary says which set went up.
    -   **Publishes the Play listing:** every promotion runs the `play_listing` lane with the tag's text for every locale and the rendered screenshots, as a dry run on closed and open and for real on production, held as "changes not sent for review". Production also opens a self-merging PR that writes the rendered screenshots back into `fastlane/`, which F-Droid and IzzyOnDroid read from git, and `desktopApp/packaging/linux/screenshots/`.

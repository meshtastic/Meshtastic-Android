...
class BurningManGoogleTileProvider(file: File) : TileProvider {
    ...
    override fun getTile(x: Int, y: Int, zoom: Int): Tile? =
        archive.tile(x, y, zoom)
            ?.let(::rasterize)
            ?.let { bytes -> Tile(TILE_SIZE, TILE_SIZE, bytes) }
            ?: TileProvider.NO_TILE

    fun covers(latitude: Double, longitude: Double): Boolean = archive.covers(latitude, longitude)

    private fun rasterize(vectorTile: ByteArray): ByteArray {
    ...
    private fun drawAreaLayer(canvas: Canvas, features: List<VectorFeature>, layer: String, color: Int) {
        ...
        features.filter { it.layer == layer && it.type == GeometryType.Polygon }.forEach { feature ->
            feature.paths.forEach { path -> canvas.drawPath(path.path, paint) }
        }
    }
    ...
    private fun drawRoad(bitmap: Bitmap, feature: VectorFeature) {
        ...
        feature.paths.forEach { path ->
            path.points.zipWithNext().forEach { (start, end) ->
                drawLine(bitmap, start, end, color, width)
            }
        ...
        for (offsetY in -radius..radius) {
            for (offsetX in -radius..radius) {
                if (offsetX * offsetX + offsetY * offsetY <= radius * radius) {
                    ...
                    if (pixelX in 0 until TILE_SIZE && pixelY in 0 until TILE_SIZE) bitmap.setPixel(pixelX, pixelY, color)
                }
            }
        }
    }
...
private class LocalPmtiles(private val file: File) {
    ...
    fun tile(x: Int, y: Int, zoom: Int): ByteArray? {
        if (zoom !in header.minZoom..header.maxZoom || x !in 0 until (1 shl zoom) || y !in 0 until (1 shl zoom)) return null
        val entry = findEntry(rootDirectory, zxyToTileId(zoom, x, y)) ?: return null
        if (entry.runLength == 0) return null // Task 2 writes a bounded root-only directory.
...
private fun parseDirectory(bytes: ByteArray): List<PmtilesDirectoryEntry> {
    ...
    return List(count) { index -> PmtilesDirectoryEntry(tileIds[index], offsets[index], lengths[index], runLengths[index]) }
}
...
private enum class GeometryType { Unknown, Point, LineString, Polygon }
...
private fun decodeValue(bytes: ByteArray): String {
    ...
    while (!reader.isAtEnd) {
        val tag = reader.readVarint().toInt()
        if (tag ushr 3 == VALUE_STRING_FIELD && tag and WIRE_TYPE_MASK == LENGTH_DELIMITED) return reader.readBytes().decodeToString()
        reader.skip(tag and WIRE_TYPE_MASK)
    }
    return ""
}
...
private fun decodeFeature(
    ...
    while (!reader.isAtEnd) {
        ...
        when (tag ushr 3) {
            ...
            FEATURE_TYPE_FIELD -> type = GeometryType.entries.getOrElse(reader.readVarint().toInt()) { GeometryType.Unknown }
...
private fun decodeTags(tags: List<Long>, keys: List<String>, values: List<String>): Map<String, String> =
    tags.chunked(2).mapNotNull { pair ->
        if (pair.size == 2) keys.getOrNull(pair[0].toInt())?.let { key -> values.getOrNull(pair[1].toInt())?.let { key to it } } else null
    }.toMap()

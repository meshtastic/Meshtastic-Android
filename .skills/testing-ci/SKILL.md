...
8. **`protobufs-bump`** — Its own workflow (`.github/workflows/protobufs-bump.yml`), on any PR that changes the `meshtastic-protobufs` line of `gradle/libs.versions.toml`. Runs `:schema-strings:sync`, pushes the regenerated `values/schema_strings.xml` onto the PR branch with the `CROWDIN_GITHUB_TOKEN` PAT (a `GITHUB_TOKEN` push would start no checks), and leaves one sticky comment from `scripts/protobufs-bump-summary.py`: the upstream compare, the merged protobufs PRs, the `.proto` delta, and the settings strings that change. Because that push is by someone other than Renovate, Renovate stops rebasing and automerging the PR; a protobufs bump is merged by a person (`automerge: false` in `renovate.json` says so too).

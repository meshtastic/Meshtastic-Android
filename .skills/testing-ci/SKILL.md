...
- **Same-repo PRs**: CI auto-records new reference images via `updateGoogleDebugScreenshotTest` and pushes them back to the PR branch using `stefanzweifel/git-auto-commit-action@v5`. Developers just need to `git pull` to get the updated references.
...
- **Merge queue / main**: CI hard-fails. References must be updated in the PR before merging.
...
- **OS guidance**: Reference images are best generated by CI (`ubuntu-24.04`), which auto-records and pushes updated references on same-repo PRs. Generating locally on macOS is acceptable because the 2% threshold absorbs rendering differences. If flakiness occurs, let CI regenerate by pushing a UI change — it will auto-update.
...
- **Updating references**: Run `./gradlew updateGoogleDebugScreenshotTest`, then commit the updated `.png` files under `app/src/screenshotTestGoogleDebug/reference/`. Alternatively, push the code change and let CI auto-update on same-repo PRs.
...
4. **Auto-update**: For same-repo PRs, CI auto-records and pushes updated references. If the summary says "References were auto-updated", just `git pull` to get them.
5. **Common causes**:

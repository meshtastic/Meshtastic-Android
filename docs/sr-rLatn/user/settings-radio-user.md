...
| Лиценцирани оператор | Enable if you hold an amateur radio license (permits higher power). Turning it on relabels **Long Name** as **Call Sign** and adds a separate Long Name field, and is staged behind a confirmation dialog |
...
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| Улога                                      | Node behavior (Client, Router, etc.) — each option carries its own description in the picker. Choosing Router asks for confirmation | Клијент       |
| Режим реемитовања                          | How the node retransmits messages; each mode is described in the picker                                                                                                                | Сви           |
| Node Info Broadcast (s) | Interval for broadcasting node info                                                                                                                                                    | 10800         |
| Double-tap Button                          | Treat a double tap as a button press                                                                                                                                                   | Онемогућено   |
...
| Override Duty Cycle              | Ignore the region's duty-cycle limit. Only legal where you are permitted to                                                                                                      | Искључен                                  |
| Игнориши MQTT                    | Drop packets that arrived from MQTT rather than over the air                                                                                                                                     | Искључен                                  |
| OK to MQTT                       | Allow your packets to be forwarded to MQTT by gateways                                                                                                                                           | Искључен                                  |

...
| Задовільний | 2    | less than 5.5 dB below `limit`                 | Decodable, but approaching the floor.                                               |
| Поганий     | 1    | 5.5 dB to 7.5 dB below `limit` | At the edge of what the preset can recover.                                         |
| Жоден       | 0    | more than 7.5 dB below `limit`                 | Far below the preset's floor; further packets from this node are likely to be lost. |

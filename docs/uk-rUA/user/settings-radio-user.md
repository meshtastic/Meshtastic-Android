...
| ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------- |
| Роль                                           | Node behavior (Client, Router, etc.) — each option carries its own description in the picker. Choosing Router asks for confirmation | Клієнт           |
| Режим ретрансляції                             | How the node retransmits messages; each mode is described in the picker                                                                                                                | Усі              |
...
| Ігнорувати обмеження завантаженості каналу | Ignore the region's duty-cycle limit. Only legal where you are permitted to                                                                                                      | Off                                       |
| Ігнорувати MQTT                            | Drop packets that arrived from MQTT rather than over the air                                                                                                                                     | Off                                       |
...
| Інтервал гортання каруселі       | How often the device cycles between screens on its own                                                                                                    |
| Режим екрану                     | Screen layout/density used by the firmware                                                                                                                |
| Одиниці виміру                   | Metric or Imperial on the device's screen                                                                                                                 |
| Використовувати 12-г формат часу | Show the device clock as 12-hour rather than 24-hour                                                                                                      |
...
| Прокидання дотиком або рухом     | Light the screen when the device is tapped or moved                                                                                                       |
...
| Setting                          | Опис                                                                                                                       |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| WiFi увімкнено                   | Enable the WiFi radio (ESP32 devices)                                                                   |
...
| Ключ для прямого повідомлення | The key used for direct-message encryption                                                                                                                                                                     |

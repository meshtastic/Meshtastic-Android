...
| 🔴   | Відключено            | No active connection. The app retries automatically, with a growing delay between attempts |
| ⚪    | Пристрій в режимі сну | The radio is in light sleep — the app is waiting for it to wake and reconnect, not failing                 |
...
### Налаштування Wi-Fi для mPWRD-OS

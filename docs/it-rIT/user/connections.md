...
| 🔴   | Disconnesso               | No active connection. The app retries automatically, with a growing delay between attempts |
| ⚪    | Il dispositivo è inattivo | The radio is in light sleep — the app is waiting for it to wake and reconnect, not failing                 |
...
### Provisioning Wi-Fi per mPWRD-OS

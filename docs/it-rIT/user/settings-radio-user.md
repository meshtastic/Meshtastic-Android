...
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| Ruolo                                      | Node behavior (Client, Router, etc.) — each option carries its own description in the picker. Choosing Router asks for confirmation | Client      |
| Modalità Ritrasmissione                    | How the node retransmits messages; each mode is described in the picker                                                                                                                | Tutti       |
...
| Ignora limite di Duty Cycle    | Ignore the region's duty-cycle limit. Only legal where you are permitted to                                                                                                      | Disattivato                               |
| Ignora MQTT                    | Drop packets that arrived from MQTT rather than over the air                                                                                                                                     | Disattivato                               |
...
| Durata di ogni schermata                   | How often the device cycles between screens on its own                                                                                                    |
| Modalità schermo                           | Screen layout/density used by the firmware                                                                                                                |
| Unità di misura visualizzata               | Metric or Imperial on the device's screen                                                                                                                 |
| Usa formato orologio 12h                   | Show the device clock as 12-hour rather than 24-hour                                                                                                      |
...
| Accendi lo schermo al tocco o al movimento | Light the screen when the device is tapped or moved                                                                                                       |
...
| Impostazione                     | Descrizione                                                                                                                |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| WiFi abilitato                   | Enable the WiFi radio (ESP32 devices)                                                                   |
...
| Modalità IPv4                    | DHCP, or a static address configured with the four fields below                                                            |
...
| Chiave per Messaggi Diretti | The key used for direct-message encryption                                                                                                                                                                     |
...
| Limite di sessione (minuti) | Maximum length of a single unlocked connection    |

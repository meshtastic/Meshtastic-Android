...
| No radio interface                     | Нет доступного радиоинтерфейса для отправки               | Check that your radio is connected and available.                                                                                                                   |
| Не удалось доставить в сеть            | Все попытки повторной отправки исчерпаны                  | Move closer, improve signal, or wait for mesh conditions to improve.                                                                                                |
| Channel/key mismatch                   | Destination channel/key does not match                    | Verify both nodes share the same channel and PSK.                                                                                                                   |
| Слишком большое сообщение для отправки | Сообщение превышает максимальный размер полезной нагрузки | Shorten the message and try again.                                                                                                                                  |
| No app response                        | App or plugin did not respond to the request              | Retry or check the destination app or module state.                                                                                                                 |
| Duty cycle limit                       | Достигнут региональный лимит эфирного времени             | Wait for the duty cycle window to reset.                                                                                                                            |

...
| 🔴    | Прекъсната връзка | No active connection. The app retries automatically, with a growing delay between attempts |
| ⚪     | Устройството спи  | The radio is in light sleep — the app is waiting for it to wake and reconnect, not failing                 |
...
### Осигуряване на Wi-Fi за mPWRD-OS

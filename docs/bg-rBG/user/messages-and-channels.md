...
| Грешка                              | Delivery failed — tap the status for the specific reason (see [Delivery Errors](#delivery-errors))                                                                                                                    |
...
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Няма маршрут                               | No path exists to the destination node                                                                                                                                        | The recipient may be offline or out of mesh range. Try later or move closer.                          |
...
| Получателят се нуждае от вашия ключ        | Direct-message encryption could not complete because the other node does not have your public key yet                                                                         | Exchange node info — the key travels with it. Common on a first DM to a new contact.                  |
| Ключът на получателя не е наличен          | You do not have the recipient's public key                                                                                                                                    | Wait for their node info to arrive, or ask them to broadcast it.                                                      |
| Не можа да се изпрати криптирано съобщение | Encryption failed for this direct message                                                                                                                                     | Verify both nodes have exchanged keys and are on compatible firmware.                                                 |
...
| Съобщението е твърде голямо за изпращане   | Message exceeds maximum payload size                                                                                                                                          | Shorten the message and try again.                                                                                    |

...
| 🔴   | Αποσυνδεδεμένο    | No active connection. The app retries automatically, with a growing delay between attempts |
| ⚪    | Συσκευή σε ύπνωση | The radio is in light sleep — the app is waiting for it to wake and reconnect, not failing                 |

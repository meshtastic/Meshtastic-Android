...
| 🔴   | מנותק           | No active connection. The app retries automatically, with a growing delay between attempts |
| ⚪    | מכשיר במצב שינה | The radio is in light sleep — the app is waiting for it to wake and reconnect, not failing                 |

...
The app reads `INFO_UF2.TXT` from the drive you select to confirm it really is the device's update drive and to identify the board before writing anything. Kui see ei suuda kinnitada, millist sinihamba ühendust seade kasutab, keeldub see kustutamast ja suunab hoopis [Web Flasheri] (https://flasher.meshtastic.org) juurde – seal valesti valides võib seade vajada taastamiseks riistvaraprogrammeerijat.

...
| -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| Rôle                                                                 | Node behavior (Client, Router, etc.) — each option carries its own description in the picker. Choosing Router asks for confirmation | Client     |
| Mode de réémission                                                   | How the node retransmits messages; each mode is described in the picker                                                                                                                | Tout       |
| Node Info Broadcast (s)                           | Interval for broadcasting node info                                                                                                                                                    | 10800      |
| Double-tap Button                                                    | Treat a double tap as a button press                                                                                                                                                   | Désactivé  |
...
| Autoriser le dépassement du temps d'émission autorisé par heure | Ignore the region's duty-cycle limit. Only legal where you are permitted to                                                                                                      | Éteint                                    |
| Ignorer MQTT                                                    | Drop packets that arrived from MQTT rather than over the air                                                                                                                                     | Éteint                                    |
| OK vers MQTT                                                    | Allow your packets to be forwarded to MQTT by gateways                                                                                                                                           | Éteint                                    |
...
| Intervalle du carrousel        | How often the device cycles between screens on its own                                                                                                    |
| Mode d'affichage               | Screen layout/density used by the firmware                                                                                                                |
| Unités d'affichage             | Metric or Imperial on the device's screen                                                                                                                 |
| Utiliser le format horaire 12h | Show the device clock as 12-hour rather than 24-hour                                                                                                      |
...
| Réveil par appui ou mouvement  | Light the screen when the device is tapped or moved                                                                                                       |
...
| Setting                          | Description                                                                                                                |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| WiFi activé                      | Enable the WiFi radio (ESP32 devices)                                                                   |
...
| Mode IPv4                        | DHCP, or a static address configured with the four fields below                                                            |
...
| Clé de message direct     | The key used for direct-message encryption                                                                                                                                                                     |
...
| Limite de sessions (minutes) | Maximum length of a single unlocked connection    |

...
| 將 .CSV 保存到內部儲存空間(僅限ESP32) | Log received test data to the radio's own filesystem. ESP32 hardware only |
...
| 重新開機     | Restarts the radio                                                                                     |
| 關機       | Powers the radio down                                                                                  |
| 恢復出廠設置   | Returns every setting to its factory default                                                           |
| 重設節點資料庫  | Clears the radio's node database. This dialog carries a **Preserve Favorites?** switch |

...
| 一般    | 2    | RSSI better than `-126 dBm` with good SNR, **OR** SNR better than `-15 dB` with good RSSI | Signal getting quieter or noisier, but still decodable. |
| 差     | 1    | Falls between Fair and None thresholds                                                    | At the edge of range or experiencing interference.      |
| 无     | 0    | RSSI worse than `-126 dBm` **AND** SNR worse than `-15 dB`                                | Transmission completely buried in noise.                |

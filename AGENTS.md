...
- Reusable CI in `.github/workflows/reusable-check.yml` is structured as four parallel job groups:
  ...
  4. **`build-desktop`** — Multi-OS matrix (`macos-latest`, `windows-latest`, `ubuntu-24.04`) that builds runnable desktop distributions via `createDistributable` (depends on `lint-check`). Note: `ubuntu-24.04-arm` is excluded because Kotlin/Native does not support `linux-aarch64` as a host platform, and KMP module configuration fails during Gradle sync.

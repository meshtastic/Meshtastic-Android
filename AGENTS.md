...
- Reusable CI in `.github/workflows/reusable-check.yml` is structured as four parallel job groups:
  ...
  4. **`build-desktop`** — Multi-OS matrix (`macos-latest`, `windows-latest`, `ubuntu-24.04`, `ubuntu-24.04-arm`) that builds runnable desktop distributions via `createDistributable` (depends on `lint-check`).

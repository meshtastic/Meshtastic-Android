...
### 2026-09-10 — Native Chrome upload recovered; 22 screenshots accepted

- Mac unlocked in the next goal continuation. Installed/launched the unsigned Release Simulator App on the dedicated App Store iPhone/iPad, checked Connection/Settings/Channels and exported six PNGs through Simulator File > Save Screen. Five replaced primary English upload assets; iPad Settings remains QA only because the unsigned Simulator has no App Group container. Also captured missing NTsocial Japanese Channels screenshot.
- Chrome extension `getTab(543619558)` still timed out/reset JS. Native `cua.getApp('Google Chrome')` exposed a stale beforeunload dialog; cancelled it. The older grouped NTsocial tab remained blank, but the other existing App Store Connect tab worked through native AX. No extension permission change was needed: native Chrome file picker uploads succeeded. Unlock/file-URL permission questions are superseded.
- Uploaded and verified 22 screenshots. Exact sets and observed order are in `release/store-screenshot-upload-result.json`. Both primary English phone/pad sets are complete; NTsocial has Chinese/Japanese iPhone overrides; MeshLink has Chinese iPhone and Japanese iPad overrides. Remaining localized dimensions inherit English. Native drag reordering failed with noWindowsAvailable, so do not claim a changed order.
- Both Add for Review preflights now clear screenshot/privacy-URL errors. MeshLink still requires published App Privacy and content rights; NTsocial also requires export compliance. No review submission was created.
- Both archive Development profiles cover the two connected iPhone 15 devices and have exact company App Group/Keychain grants. Read-only devicectl inventory shows NTsocial 1.0.0 (1) on both; no physical install/launch/data change. Asked which devices/radios are eligible for fresh, history-isolated Gateway testing under parent AGENTS.md lines 1961-1962.
- Company/privacy/rights/France/DSA answers remain pending for three consecutive goal turns. All available mandatory screenshot preparation/upload work is complete; further submission and physical testing need user input. Do not count waiting as consent.
- Current JS bindings: `chromeApp` (native Google Chrome); helpers `prepareStoreUpload(path)`, `finishStoreUpload()`, `selectStoreLocale(name)`. `simulatorApp` and save helper were lost in a browser timeout reset; reacquire when needed. Current Chrome working tab is MeshLink Japanese iPad media manager, two custom screenshots saved. The old extension-oriented `storePage` is unavailable; do not keep retrying it while native AX works.
- Native picker workflow: choose enabled file button, Cmd+Shift+G, enter full first file path, Return, then observe again until Open enabled. Shift+Down extends selection; Cmd+A was ineffective. Multi-file uploads may arrive in a different order. Header-only content after direct media URL navigation resolved by opening the app's distribution/version page first and then clicking its media-manager link. No new build jobs remain.


## 2026-09-10 — Company approvals executed; DSA verification pending

- User explicitly approved every pending item and instructed final submission of both apps. This covers privacy publication, third-party content rights, excluding France for NTsocial, LiberaNt LLC DSA trader/public contacts, and fresh device/radio tests. Do not ask these permissions again.
...
- User then declined sending the support request, concerned it would stop App Review. Do not send it or treat the earlier pending authorization as approval. Explained that a support request is distinct from cancelling review; both submissions were read again and still show Waiting for Review. No cancellation or resubmission action occurred. Tab 2 (`liveReviewTab` / `liveReviewPage`) now shows MeshLink's existing submission. DSA phone verification remains unresolved; Apple documents the trader-verification availability restriction for the EU, not proof that these existing review submissions stopped.


## 2026-09-11 — Five-phone MeshLink clean install from current source
- User explicitly requested removal of old MeshLink from all five phones, verification of absence on all five, then installation of the latest local project. Source HEAD `fdb4d12021023a53580902eb718f78fdcb5f27fd`; product worktree initially clean.
- Removed `com.ntsocial.meshlink.google.debug` on OPPO CPH2695 (USB), Samsung SM-S9280 and SM-S9080 (Wi-Fi ADB); removed `com.ntsocial.meshlink.ios` on iPhone UDID ending C0201E. The other iPhone ending D1A01E was already absent, including an all-apps inventory. All five absence checks passed before any new install. NTsocial parent apps remained installed.
- JDK 21/Android SDK bootstrap and proto init completed. Android `:app:assembleGoogleDebug --no-configuration-cache` passed (450 tasks), producing 1.0.8 (9), arm64-v8a, APK SHA-256 `eb0d3a67ea07509777ff9bdb526f56dd803b683d57e11046f4453ee91505afbc`; APK signature verified. Installed without replace/keep-data on three phones, matching version and equal fresh firstInstallTime/lastUpdateTime read back. All launched to the initial three-language selection screen; foreground and live PIDs verified.
- Fresh Xcode Debug iphoneos build passed for iOS 1.0.0 (2); strict/deep codesign and actual entitlements/profile verified organization Team D524H699HW, original bundle, App Group `group.com.ntsocial.gateway`, shared Keychain suffix, valid expiry and coverage of both phones. Development signature (`get-task-allow=true`), executable SHA-256 `9b6b051bcba4065ea37a78f906ef918d4bb096c495b8a5e64260e5dedef7546f`. Both Wi-Fi installs and version readbacks passed; both launched and retained the same PIDs approximately 25 seconds later.
- Evidence: `.agent_plans/five-phone-reinstall-20260911/REPORT.md` and its JSON/log/Android screenshot artifacts. OS app uninstall does not claim shared App Group/Keychain erasure; those parent-shared stores were preserved. No radio configuration/message send, BLE/RF/Gateway validation, iOS touch automation, Windows work, product source change, commit or push was performed.

## 2026-09-11 — iOS MeshLink Guideline 2.1 reply preparation
- User requested a sincere, accessible review-reply draft including `https://youtube.com/shorts/7UuDxVEG1u4`; no reply or review-notes changes were submitted.
- Read the signed-in App Store Connect submission for MeshLink app 6810479958, submission `9f715b15-67f5-4379-b79c-1eae18d2192c`: version 1.0.0 (2), Rejected / unresolved, Guideline 2.1 Information Needed, Apple message September 11 at 13:21 local. This supersedes the earlier Waiting for Review status for this submission only.
- YouTube Studio identifies the provided 1:19 video as `iOS NTsocial app (operation demo)`, unlisted. Playback observations show iPhone Control Center at the start and the separate NTsocial Public/People interface, including images and profile/block controls. It is not verified as a MeshLink radio walkthrough. Asked user for the MeshLink recording or timestamp and the recorded device/OS/build; these facts remain unconfirmed at this point. Browser automation subsequently stopped because another extension UI was open; no bypass attempted.
- Checked current iOS metadata/runtime and shared controls: standalone radio use needs no account or companion app; compatible external Meshtastic BLE hardware is required. iOS analytics, MQTT proxy, and remote device/firmware API implementations are no-op. Optional primary-radio NTsocial integration must remain distinguished from standalone use. User states the app is free and does not require personal registration information; this is not a claim that radio messages/identifiers never leave the phone.
- Source has node Ignore and configurable Message Filter, including ignore administration/filter updates. Focused iOS/shared source/resource search found no in-app content/user abuse-report submission workflow. Do not claim no UGC, an implemented report mechanism, or an automatic Guideline 1.2 exemption because communication is account-free or decentralized. The supplied video cannot establish controls in the separate MeshLink product.
- Drafting scope is iOS review communication only; no Android/Windows/product implementation changes, build, device QA, store submission, commit, or push. Any future reply must distinguish recorded evidence from unconfirmed latest-OS/submitted-artifact claims and include the requested information in both the reply and App Review Notes when authorized.
- Follow-up: user corrected the MeshLink video to `https://youtube.com/shorts/09h6s7sN2O0`. This replaces the earlier NTsocial video in the MeshLink reply. Browser playback verified title `iOS NTsocial MeshLink app (Operation Demo)` and sampled frames: initial iPhone Control Center, first-language selection around 00:05, Bluetooth discovery around 00:15, connecting around 00:45, node Details/Ignore control, connected Channels around 01:46, and Settings with MeshLink bundle identity around 02:32. Recorded device model, exact OS version, and build identity remain unconfirmed; these observations do not establish firmware/RF delivery or complete submitted-build QA. Updated English reply uses the correct video and no longer needs a placeholder video URL. No App Store Connect reply or Notes update was sent.

## 2026-09-16 — Five-phone parent/MeshLink integration audit paused for manual pairing
- User requested current Android/iOS parent-to-MeshLink communication/control real-device testing on five phones. Three Android devices and two iPhone 15 devices inventoried, with duplicate ADB transports deduplicated. Android parent 1.6.1 (41), MeshLink 1.0.8 (9); iOS parent 1.0.0 (1), MeshLink 1.0.0 (2), matching company App Group visible for both apps. Product source/worktrees initially clean.
- Initial onboarding/unpaired states were observed. During user manual configuration B (S24 Ultra) showed 0809 and 5d6e endpoints and parent MeshLink ready; I1 (UDID ending D1A01E) showed connected 1407 and eight parent catalog projections. These observations are not local admission or RF delivery proof. Source still contains the documented Android secondary v3 send mismatch and live history-epoch reset defect.
- User confirmed they are manually pairing and asked agent to take over after completion. All device operations stopped; no test message, history clear, product reinstall or product source change. Resume only on user completion. Exact resume/evidence notes: `.agent_artifacts/meshlink-integration-20260916/RESUME.md`.
- iOS status JSON initially looked active but later retained identical timestamp/old PID: it is stale evidence, not verified fresh app control. New heartbeat/exact response remains required. A test-only XCUITest auxiliary runner was installed over the existing wirelessprobe runner; restore the original parent's helper at final handoff as detailed in RESUME.md. Private device evidence stays ignored.


## 2026-09-16 — Five-phone parent/MeshLink audit resumed and documented
- User said「可以接手」after manual pairing. Completed a bounded audit on A OPPO, B S24 Ultra, C S22 Ultra, I1/I2 iPhone 15. Only B (0809 + 5d6e) and I1 (1407) had radio pairings; A/I2 were unselected and C remained onboarding. Report: `docs/meshlink-parent-five-phone-audit-2026-09-16.md`. No product code changes/build/install, no history deletion, no PUBLIC test sends, no commit/push. Auxiliary test harness was built/installed only; the original parent wirelessprobe app/runner was restored on both iPhones.
- PASS native RF: one B-primary parent UI message `NTS0907-0916-B-I1-N01`, packet 21997241, stable ID F80F5235385013E5D486604B54CFD768. B primary DELIVERED; I1 and B secondary RECEIVED, matching native text/ID and viaMqtt=false. This is not full iOS parent native import/UI proof.
- FAIL secondary native: `NTS0907-0916-B2-I1-N02`, parent message C36BF61E3C7A15AA43286D101DD57B73, remains in B outbox on secondary endpoint 12e2c5ba-809d-49f9-88f7-ce2fefbec936. No matching durable native packet found. Current production source still advertises native/overlay send through MeshtasticEndpointGatewaySource while injecting SecondaryGatewayRepository whose sends throw. Secondary cacheInbound and inbound activation also return false (source evidence, not a parent ingress device pass).
- iOS native UI gap: 8 projections exist, but parent normal list is joinedChannels only; enqueueNativeBroadcastText production UI caller not found (test calls only). A new encrypted native board file is not proof of exact parent canonical import. Android native channel also displays overlay-only explanation: source + UI mismatch.
- iOS overlay `NTS0907-0916-I1-B-O03` canonical FBA6A046A05D26C9C6D7B97FD368D529 was verified through a fresh inspector response, one committed/visible row and exact caption SHA. MeshLink private ledger ACCEPTED packet IDs 1903220361, 516578448, 257389697, 277276486; all remain QUEUED at final check. B radio FromRadio logs have none of those IDs. I1 initially restored to Connected/parent READY, then explicit same-radio disconnect/connect/Continue and one MeshLink restart ended Error/Not connected. User was asked to confirm/restart 1407; no reply yet. Hardware vs app cause remains unresolved, do not claim HMAC failure or complete RF.
- Android online overlay O04 correctly used cloud-first. Temporarily disabled only B parent's Internet switch for O05 fallback. Primary persisted 20 port-256 ENROUTE packets, and secondary FromRadio log has all 20 exact IDs and full payload SHA-256 matches, transport=LoRa and viaMqtt=false. Do not infer failed overlay RX from absence in `packet`: MeshDataHandler sends PRIVATE_APP ingress to Gateway cache; use `log.from_radio`. The offline transition can trigger due work; do not assert every packet is a fragment of only O05. RF success is two radios on B, not remote parent canonical import.
- RESTORED: B Internet ON with fresh runtime ready; A/C ready. Test-only social-channel LoRa bindings removed on B and I1; original native projections and radio pairing data retained. Queued test rows retained as evidence and may later drain. Both iOS parent controls restored via original XCUITest launchControl (devicectl-only launches earlier left I1 stale). Exact new status responses I1 FEA4CDC4-BB74-40E7-873D-E7F1AD761E03 and I2 6418C4EA-DBFC-41AA-9DFC-E8C65E27F578; PIDs 4422/16726, advancing heartbeats, Internet ready, BLE Running, highSpeedEnabled true, usingTestRelay false, auto-send/provision/capture disabled. Final I1 radio still Error, not restored to RF-ready.
- Private ignored evidence `.agent_artifacts/meshlink-integration-20260916/` includes sanitized summaries and private raw UI/DB/WAL/logs. Never print private settings, PSKs, tokens, unrelated message content or decrypt history. Apps remain running. Product sources unchanged: MeshLink 3c5cb824d; parent be7299ef2. Future work: fix Android secondary capability/wiring, add iOS native UI, restore 1407 and diagnose accepted queue drain/QueueStatus, then isolate non-LoRa transports for complete cross-parent two-way import proof.


## 2026-09-16 — Authorized Gateway repair and five-phone upgrades
- User explicitly authorized deeper diagnostics, source repair, tests, build, physical upgrades and retesting. This section supersedes the preceding audit's unresolved secondary-send/iOS queue/native-UI findings; preserve that earlier failure evidence. MeshLink branch `codex/gateway-device-repair-20260916` on `3c5cb824d`; external parent branch `codex/meshlink-native-integration-20260916` on `be7299ef2`, both dirty/uncommitted. No subagents, commits, push, PUBLIC sends, history/account reset or radio PSK/config change.
- Android secondary v3 now resolves isolated production NtsocialGatewayRepositoryImpl plus SessionMessageQueue, scoped Room/ingress gate/transport; root legacy WorkManager stays unchanged. Shared queue retains startup signals, exact-session/source/QueueStatus validation, native-first lane and 30-second transient retry. Extracted existing iOS queue suppression annotations were narrowed to the new drain rather than adding blanket lint bypass.
- iOS permanent QUEUED root cause was DI: a default constructor `GatewayIngressSessionGate()` caused Koin-generated constructor code to omit the dependency and allocate a second gate. Device logs showed active repository gate but inactive queue gate; generated bytecode and a production composition regression reproduced it. Mandatory injection fixes it. Old O03's same four packet IDs drained automatically and matched RF payloads at both B radios. BLE cancel/recovery instrumentation was added, not a replacement Swift radio stack.
- Parent Android had a second send defect: current/captured route-token string equality treated ordinary catalog token refresh as route change, interrupting O08 mid-transfer. Preflight now compares the complete route descriptor/uplink/downlink context, retaining the captured token and authoritative provider TTL/caller/source/endpoint/generation admission. Unit boundaries cover benign refresh and actual context changes.
- Parent Android also lost second-endpoint native history because global Seen IDs were treated as per-projection durable duplicates. N07 had one primary row and zero secondary rows despite both radio receipts. Import now dedupes within the authorized projection and retains discard/provenance restrictions. Private endpoint cursors carry projectionDedupeVersion=1; older cursors replay retained rows once without deleting history. Device upgrade recovered N07 to one row in each board; new N11 likewise imported once per board.
- Parent iOS now has separate native projection list/local canonical history/180-byte UTF-8 composer using the Apple Gateway native sink, fresh route resolution and stable draft retry IDs. Own echo comes from the insertion feed; no social membership or BLE/Cloud/history-sync authority is added. Android native channel's overlay-only notice was corrected. Xcode27 required expressing the existing Cloud challenge signature bytes as sequential append rather than a giant Data + expression; bytes/protocol remain unchanged.
- RF evidence: N06 B secondary -> I1 native packet1071336318/sourceID75DE3EDBFB36D9BEE0B16FE45B3D8083; N07 I1 -> B packet1711983053/sourceID131EF434E823C028DFAF80315D0163AE; O09 I1 four port256 packets, B canonical rx_commit transport=meshtastic and exact decrypted hash; O10 final token fix one transfer, 20/20 admitted and 20/20 matching LoRa packets at I1, complete receipt and one cold-restarted parent committed row32CEE37AE025AB9B6CC8BFBCCC047822. O08's original route-changed failure and recovery remain in evidence. N11 final native packet1751315345/sourceIDB5D7962563D9E11D62B0AA307204FC4F: I1 DELIVERED, both B radios RECEIVED, both mother projections count1. Native is local-radio-only; overlay RF/reassembly and canonical commit are separately stated where other bearers may coexist.
- MeshLink final whole gate 1,947 tasks SUCCESS, plus signed iOS device and unsigned simulator build/launch; Kotlin Native executable tests remain disabled, so no native runtime test execution claim. Parent final Android 1,148 app tests, 3 skipped, no errors/failures; UTFind19. Focused Apple Gateway46 pass. Final full iOS gate at parent work/meshlink-repair-20260916/ios-parity-frozen completed 2026-09-16 11:51:39 Asia/Taipei, 14/14 steps passed: Swift897/3skip/0failure, cross-runtime exchange, Release build, UI11/0fail, source-state-unchanged. Evidence remains local_passed_external_required, not Store/hardware-wide release signoff. UI xcresult retains two Invalid frame dimension runtime warnings. Earlier UI gates had 11 passing cases but one Xcode diagnostics timeout/interruption and one deliberate source-state-unchanged failure during continued repairs. Final wrapper disables only extra sysdiagnose collection, never assertions. Only documentation changed after this gate; all31 affected product/test source hashes match the retained manifest.
- All five product app upgrades retain original identities; final Android parent uses remoteFileEnabled=true and iOS NTSOCIAL_REMOTE_FILE_ENABLED=YES. Installed artifact/source hashes are in .agent_artifacts/gateway-repair-20260916/logs. iOS company TeamD524H699HW signed both apps with matching group.com.ntsocial.gateway and shared Keychain. Debug evidence is not Store acceptance.
- Handoff verified: Production, Internet ready, OS Bluetooth, high-speed and remote-file feature ON, parent foreground on all five, each sees four peers. Both iPhones have exact new control replies and advancing heartbeat using restored original DeviceControlUITests helper/runner; no auto sends/provisioning/capture. Temporary B/I1 social bindings cleared; B still paired0809+5d6e, I1 still1407. A/C/I2 have no selected radio; C still USER_FIXED denied MeshLink BLUETOOTH_SCAN/CONNECT. Never claim five-radio RF coverage. The first handoff snapshot caught I1 background after the intended MeshLink route wake; parent foreground was restored and all-green checks repeated.
- Reports: docs/meshlink-parent-gateway-repair-2026-09-16.md and external parent docs/meshlink-gateway-repair-2026-09-16.md. Private raw UI, DB/WAL, logcat and helper files stay ignored. Existing live whole-history-clear epoch defect, retired iOS scanner callback candidate, four-radio/long-background/control-mutation/Windows/Store gates remain outside this proof. No extra physical send is needed after N11.

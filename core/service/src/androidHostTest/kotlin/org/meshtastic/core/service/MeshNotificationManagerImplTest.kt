...
class MeshNotificationManagerImplTest {
    ...
    fun `service state seeds local stats before the local node row is available`() = runWithRenderScope { renderScope ->
        val stats = LocalStats.Builder().also { wb ->wb.uptime_seconds = 1; wb.num_online_nodes = 2; wb.num_total_nodes = 3}.build()
    ...
    private fun populatedTelemetry() =
        Telemetry.Builder().also { wb ->wb.local_stats = LocalStats.Builder().also { wb ->wb.uptime_seconds = 1; wb.num_online_nodes = 1; wb.num_total_nodes = 1}.build()}.build()

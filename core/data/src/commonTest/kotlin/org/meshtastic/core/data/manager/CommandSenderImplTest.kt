...
class CommandSenderImplTest {
    ...
    fun sendAdmin_generatesNonZeroIdWhenCallerSuppliesZero() = runTest {
        ...
        commandSender.sendAdmin(DEST_NODE, requestId = 0) { AdminMessage.Builder().also { wb ->wb.get_owner_request = true}.build() }
    ...
    fun sendAdminSurfacesQueueRejection() = runTest {
        ...
        assertFailsWith<PacketQueueRejectedException> { commandSender.sendAdmin(DEST_NODE) { AdminMessage.Builder().build() } }
    }
    ...
    fun sendAdminForConnectionForwardsLifecycleOwnershipToAdmission() = runTest {
        ...
        commandSender.sendAdminForConnection(DEST_NODE, expectedConnectionVersion = 17L) { AdminMessage.Builder().build() }
    ...
    fun sendAdminAwaitResult_generatesNonZeroIdWhenCallerSuppliesZero() = runTest {
        ...
        commandSender.sendAdminAwaitResult(DEST_NODE, requestId = 0) { AdminMessage.Builder().also { wb ->wb.get_owner_request = true}.build() }
    ...
    fun sendAdminAwaitResult_preservesNonAcceptedStatus() = runTest {
        ...
        val result = commandSender.sendAdminAwaitResult(DEST_NODE) { AdminMessage.Builder().also { wb ->wb.get_owner_request = true}.build() }
    ...
    fun sendAdminImmediate_dispatchesDirectToRadioWithPasskeyAndNoResponse() {
        ...
        commandSender.sendAdminImmediate(DEST_NODE) { AdminMessage.Builder().also { wb ->wb.set_time_only = 12345}.build() }
    ...
    fun requestNeighborInfo_localNode_usesCachedNeighborInfo() = runTest {
        val cached = NeighborInfo.Builder().also { wb ->wb.node_id = MY_NODE_NUM; wb.last_sent_by_id = MY_NODE_NUM}.build()
    ...
    fun sendPosition_updatesLocalPositionWhenNotFixed() = runTest {
        ...
        val pos = org.meshtastic.proto.Position.Builder().also { wb ->wb.latitude_i = 10000000; wb.longitude_i = 20000000}.build()
    ...
    fun sendPosition_rejectsWhenLocalNodeIdentityIsUnavailable() = runTest {
        ...
        assertFailsWith<LocalNodeUnavailableException> { commandSender.sendPosition(org.meshtastic.proto.Position.Builder().build()) }
    ...
    fun sendPosition_skipsLocalUpdateWhenFixedPosition() = runTest {
        ...
        val configFlow =
            MutableStateFlow(LocalConfig.Builder().also { wb ->wb.position = org.meshtastic.proto.Config.PositionConfig.Builder().also { wb ->wb.fixed_position = true}.build()}.build())

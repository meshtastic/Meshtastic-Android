...
class StoreForwardPacketHandlerImplTest {
    ...
    fun `handleStoreAndForward text adopts original_id as the packet id`() = testScope.runTest {
        ...
        val sf =
            StoreAndForward.Builder()
                .also { wb ->
                    wb.text = "Replayed by a router".encodeToByteArray().toByteString()
                    wb.rr = StoreAndForward.RequestResponse.ROUTER_TEXT_DIRECT
                    wb.original_id = 0x4321
                }
                .build()
        val packet = makeSfPacket(999, sf)
        val dataPacket = makeDataPacket(999)
    ...
    fun `handleStoreAndForward text keeps the header id when original_id is absent`() = testScope.runTest {
        ...
        val sf =
            StoreAndForward.Builder()
                .also { wb ->
                    wb.text = "Replayed by an older router".encodeToByteArray().toByteString()
                    wb.rr = StoreAndForward.RequestResponse.ROUTER_TEXT_DIRECT
                }
                .build()
        val packet = makeSfPacket(999, sf)
    ...
    fun `handleStoreAndForward text ignores original_id for a non-router-text rr`() = testScope.runTest {
        ...
        val sf =
            StoreAndForward.Builder()
                .also { wb ->
                    wb.text = "Text under an unexpected response type".encodeToByteArray().toByteString()
                    wb.rr = StoreAndForward.RequestResponse.ROUTER_ERROR
                    wb.original_id = 0x4321
                }
                .build()
        val packet = makeSfPacket(999, sf)

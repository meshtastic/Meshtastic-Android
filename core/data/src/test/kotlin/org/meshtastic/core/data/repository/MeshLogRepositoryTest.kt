...
class MeshLogRepositoryTest {
    ...
    fun `parseTelemetryLog preserves zero temperature`() = runTest(testDispatcher) {
        val zeroTemp = 0.0f
        val envMetrics = EnvironmentMetrics(temperature = zeroTemp)
        val telemetry = Telemetry(environment_metrics = envMetrics)

        val meshPacket =
            MeshPacket(
                decoded =
                Data(
                    payload = Telemetry.ADAPTER.encodeByteString(telemetry),
        ...
        val result = repository.parseTelemetryLog(meshLog)
        assertNotNull(result)
        val resultMetrics = result?.environment_metrics
        assertNotNull(resultMetrics)
        assertEquals("Temperature should be 0.0f, not NaN", zeroTemp, resultMetrics?.temperature)
    }

    @Test
    fun `parseTelemetryLog maps null temperature to NaN`() = runTest(testDispatcher) {
        val envMetrics = EnvironmentMetrics(temperature = null)
        val telemetry = Telemetry(environment_metrics = envMetrics)
        ...
        // Should be NaN as per original intent for missing values
        assertEquals(Float.NaN, resultMetrics?.temperature)
    }
}

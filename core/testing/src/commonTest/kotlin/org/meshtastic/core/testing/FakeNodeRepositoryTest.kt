...
class FakeNodeRepositoryTest {
    ...
    fun `getUnknownNodes returns nodes with UNSET hw_model`() = runTest {
        val node1 = Node(num = 1, user = User.Builder().also { wb ->wb.hw_model = org.meshtastic.proto.HardwareModel.UNSET}.build())
        val node2 = Node(num = 2, user = User.Builder().also { wb ->wb.hw_model = org.meshtastic.proto.HardwareModel.TLORA_V2}.build())
    ...
    fun `insertMetadata updates node metadata`() = runTest {
        ...
        val metadata = org.meshtastic.proto.DeviceMetadata.Builder().also { wb ->wb.firmware_version = "2.5.0"}.build()

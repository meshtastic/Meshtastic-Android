...
class RepositoryFakesTest {
    ...
    fun `FakeTracerouteSnapshotRepository roundtrips positions keyed by log uuid`() = runTest {
        ...
        val positions = mapOf(1 to Position.Builder().also { wb ->wb.latitude_i = 10}.build(), 2 to Position.Builder().also { wb ->wb.latitude_i = 20}.build())
    ...
    fun `FakeRadioController preserves configuration and fixed-position evidence until reset`() = runTest {
        ...
        val admin = Config.Builder().also { wb ->wb.lora = Config.LoRaConfig.Builder().also { wb ->wb.hop_limit = 5}.build()}.build()
        val module = ModuleConfig.Builder().also { wb ->wb.serial = ModuleConfig.SerialConfig.Builder().also { wb ->wb.enabled = true}.build()}.build()
    ...
    fun `FakeRadioController uses null destination consistently for local config writes`() = runTest {
        ...
        val moduleConfig = ModuleConfig.Builder().also { wb ->wb.serial = ModuleConfig.SerialConfig.Builder().also { wb ->wb.enabled = true}.build()}.build()
    ...
    fun `FakeCommandSender records connection ownership on post-handshake requests`() = runTest {
        ...
        sender.sendAdminForConnection(destNum = 123, expectedConnectionVersion = 41, requestId = 7) { AdminMessage.Builder().build() }
    ...
    fun `FakeRadioController scopes standalone hooks per write`() = runTest {
        ...
        val transaction = async {
            controller.editSettings(destNum = 7) {
                ...
                setModuleConfig(ModuleConfig.Builder().also { wb ->wb.serial = ModuleConfig.SerialConfig.Builder().also { wb ->wb.enabled = true}.build()}.build())
            }
        }
        transactionStarted.await()

        controller.setConfig(destNum = 8, config = Config.Builder().also { wb ->wb.power = Config.PowerConfig.Builder().build()}.build(), packetId = 1)
        controller.setModuleConfig(
            destNum = 8,
            config = ModuleConfig.Builder().also { wb ->wb.mqtt = ModuleConfig.MQTTConfig.Builder().also { wb ->wb.enabled = true}.build()}.build(),
            packetId = 2,
        )
        ...
        assertEquals(
            listOf(
                FakeRadioController.ConfigWrite(destination = 8, config = Config.Builder().also { wb ->wb.power = Config.PowerConfig.Builder().build()}.build()),
                FakeRadioController.ConfigWrite(destination = 7, config = Config.Builder().also { wb ->wb.device = Config.DeviceConfig.Builder().build()}.build()),
            ),
            controller.configWrites,
        )
        assertEquals(
            listOf(
                ...
                FakeRadioController.ModuleConfigWrite(
                    destination = 7,
                    config = ModuleConfig.Builder().also { wb ->wb.serial = ModuleConfig.SerialConfig.Builder().also { wb ->wb.enabled = true}.build()}.build(),
                ),
            ),
            controller.moduleConfigWrites,
        )
    ...
    fun `FakeRadioController records owner destinations consistently`() = runTest {
        val controller = FakeRadioController()
        val localOwner = User.Builder().also { wb ->wb.id = "!00000001"; wb.long_name = "Local"}.build()
        val remoteOwner = User.Builder().also { wb ->wb.id = "!00000002"; wb.long_name = "Remote"}.build()
    ...
    fun `FakeRadioConfigRepository tracks channel set and module config`() = runTest {
        ...
        repo.updateChannelSettings(Channel.Builder().also { wb ->wb.index = 1; wb.settings = ChannelSettings.Builder().also { wb ->wb.name = "B2"}.build()}.build())

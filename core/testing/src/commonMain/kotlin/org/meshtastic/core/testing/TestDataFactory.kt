...
object TestDataFactory {
    ...
    fun createTestNode(
        ...
        val user = User.Builder().also { wb ->wb.id = userId; wb.long_name = longName; wb.short_name = shortName; wb.hw_model = hwModel}.build()
        val metrics = org.meshtastic.proto.DeviceMetrics.Builder().also { wb ->wb.battery_level = batteryLevel}.build()
        return Node(
            num = num,
            user = user,
            lastHeard = lastHeard,
    ...
    fun createTestPacket(
        ...
        relayNode: Int = 0,
    ) = org.meshtastic.proto.MeshPacket.Builder().also { wb ->wb.from = from; wb.to = to; wb.decoded = decoded; wb.relay_node = relayNode}.build()

 ...
 */
class InstallProfileUseCase @Inject constructor(
    ...
    suspend operator fun invoke(destNum: Int, profile: DeviceProfile, currentUser: User?) {
        with(profile) {
            ...
            if (long_name != null || short_name != null) {
                currentUser?.let {
                    val user = it.copy(
                        long_name = long_name ?: it.long_name,
                        short_name = short_name ?: it.short_name
                    )
            ...
            if (fixed_position != null) {
                radioController.setFixedPosition(destNum, Position(fixed_position!!))
            }

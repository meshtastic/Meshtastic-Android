...
class SharedContactTest {
    ...
    fun testSharedContactUrlRoundTrip() {
        val original = SharedContact.Builder().also { wb ->wb.user = User.Builder().also { wb ->wb.long_name = "Suzume"; wb.short_name = "SZ"}.build(); wb.node_num = 12345}.build()
    ...
    fun testWwwHostIsAccepted() {
        val original = SharedContact.Builder().also { wb ->wb.user = User.Builder().also { wb ->wb.long_name = "Suzume"}.build(); wb.node_num = 12345}.build()

 ...
 *
 * Before 0.3.0, the wire-to-SDK and SDK-to-wire converters in TakV2Compressor.kt
 * had no cases for [TakTalkMessage] or [TakTalkRoomData].  m-t-t and y- events
 * therefore fell through to [TakPacketV2Data.Payload.None] on the receive side,
 * which surfaced as TAKTALK voice push-to-talk messages never reaching the TTS
 * stage on the receiving ATAK device.  Chat (b-t-f) was wired but its TAKTALK
 * sidecars (lang, room_id, voice_profile_id) were dropped too.
 *
 * These tests pin the round-trip so any future regression of the same adapter
 * gaps fails fast at unit-test time instead of in the field.
 */
class TakV2CompressorTaktalkTest {

    @Test
    fun `m-t-t taktalk message round-trips with voice marker`() {
        val original = TAKPacketV2(
    ...
    fun `m-t-t with marti round-trips dest callsigns and survives compression`() {
        val original = TAKPacketV2(
            ...
            marti = org.meshtastic.proto.Marti(
                dest_callsign = listOf("ETHEL"),
            ),
        )
        ...
        assertEquals(listOf("ETHEL"), decompressed.marti!!.dest_callsign,

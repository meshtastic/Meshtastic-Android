...
/**
 * Covers the Preferences-keyed cross-transport DB-claim helpers behind [DatabaseManager.associateDevice]. Lives in
 * `nonWebTest` because `androidx.datastore:datastore-preferences` has no wasmJs variant. See `DeviceIdentityTest.kt` in
 * `commonTest` for the portable [validDeviceIdOrNull] coverage.
 */
class DeviceIdentityPrefsTest {

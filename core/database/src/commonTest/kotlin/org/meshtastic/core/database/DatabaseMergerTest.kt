...
class DatabaseMergerTest {
    ...
    private fun node(num: Int, notes: String = "") = NodeEntity(
        num = num,
        user = User.Builder().also { wb ->wb.id = "!$num"; wb.long_name = "Node $num"; wb.hw_model = HardwareModel.TBEAM}.build(),
        notes = notes,
    )
    ...
    fun mergeFoldsSecondaryTransportIntoCanonical() = runTest {
        ...
        dest
            .nodeInfoDao()
            .upsert(MetadataEntity(num = 10, proto = DeviceMetadata.Builder().build(), timestamp = 1000)) // newer than src
        ...
        source
            .nodeInfoDao()
            .upsert(MetadataEntity(num = 10, proto = DeviceMetadata.Builder().build(), timestamp = 500)) // older → dest wins
        source
            .nodeInfoDao()
            .upsert(MetadataEntity(num = 30, proto = DeviceMetadata.Builder().build(), timestamp = 500)) // dest lacks → added
        val srcSession =
            source
                .discoveryDao()
                .insertSession(

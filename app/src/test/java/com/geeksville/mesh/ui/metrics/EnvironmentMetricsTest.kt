...
class EnvironmentMetricsTest {
    ...
    fun `temperature and soil temperature are converted to Fahrenheit when isFahrenheit is true`() {
        ...
        val processedTelemetries =
            if (isFahrenheit) {
                data.map { tel ->
                    val temperatureFahrenheit = celsiusToFahrenheit(tel.environment_metrics?.temperature ?: 0f)
                    val soilTemperatureFahrenheit = celsiusToFahrenheit(tel.environment_metrics?.soil_temperature ?: 0f)
                    tel.copy(
                        environment_metrics =
                        (tel.environment_metrics ?: EnvironmentMetrics()).copy(

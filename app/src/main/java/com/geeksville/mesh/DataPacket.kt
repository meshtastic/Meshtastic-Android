...
data class DataPacket(
    ...
    val hopsAway: Int // this isn't being called... 
        get() {
            return when {
                hopStart == null && hopLimit == null -> {
                    android.util.Log.d("DataPacket", "hopsAway: No info, won't propagate: has to be 0")
                    0
                }
                hopStart == 0 || (hopLimit != null && hopStart != null && (hopLimit > hopStart)) -> {
                    android.util.Log.d("DataPacket", "hopsAway: Insufficient info, returning -1")
                    -1
                }
                hopStart != null && hopLimit != null -> {
                    android.util.Log.d("DataPacket", "hopsAway: Regular calculation, hopStart=$hopStart, hopLimit=$hopLimit, result=${hopStart - hopLimit}")
                    hopStart - hopLimit
                }
                else -> {
                    android.util.Log.d("DataPacket", "hopsAway: Fallback, returning -1")
                    -1
                }
            }
        }

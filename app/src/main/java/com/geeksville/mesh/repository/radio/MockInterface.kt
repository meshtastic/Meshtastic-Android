...
constructor(
    ...
    override fun handleSendToRadio(p: ByteArray) {
        ...
        sendQueueStatus(pr.packet?.id ?: 0)

        val data = pr.packet?.decoded

        when {
            (pr.want_config_id ?: 0) != 0 -> sendConfigResponse(pr.want_config_id!!)
            data != null && data.portnum == PortNum.ADMIN_APP ->
                handleAdminPacket(pr, AdminMessage.ADAPTER.decode(data.payload))
            pr.packet != null && pr.packet!!.want_ack -> sendFakeAck(pr)
    ...
    private fun handleAdminPacket(pr: ToRadio, d: AdminMessage) {
        ...
        when {
            ...
            (d.get_channel_request ?: 0) != 0 ->
                sendAdmin(packet.to, packet.from, packet.id) {
                    val req = d.get_channel_request!!
                    copy(
                        get_channel_response =
                        ProtoChannel(
                            index = req - 1, // 0 based on the response
                            settings = if (req == 1) Channel.default.settings else null,
                            role = if (req == 1) ProtoChannel.Role.PRIMARY else ProtoChannel.Role.DISABLED,
                        ),
                    )
                }
    ...
    private fun makeTextMessage(numIn: Int) = FromRadio(
        ...
        MeshPacket(
            ...
            Data(
                ...
                payload = "This simulated node sends Hi!".encodeToByteArray().toByteString(),
            ),
        ),
    )
        ...
        val p =
            makeDataPacket(
                ...
                Data(
                    ...
                    payload = AdminMessage().initFn().encode().toByteString(),
                    request_id = reqId,
                ),
            )
    ...
    private fun sendConfigResponse(configId: Int) {
        ...
        val packets =
            arrayOf(
                // MyNodeInfo
                FromRadio(my_info = MyNodeInfo(my_node_num = MY_NODE)),
                FromRadio(

...
constructor(
    ...
    private fun onRadioConnectionState(newState: ConnectionState) {
        scope.handledLaunch {
            ...
            val lsEnabled = localConfig.power?.is_power_saving ?: false || isRouter
    ...
    fun onHasSettings() {
        ...
        scope.handledLaunch {
            val moduleConfig = radioConfigRepository.moduleConfigFlow.first()
            historyManager.requestHistoryReplay("onHasSettings", myNodeNum, moduleConfig.store_forward, "Unknown")
        }

        // Set time
        commandSender.sendAdmin(myNodeNum) {
            copy(set_time_only = (System.currentTimeMillis() / MILLISECONDS_IN_SECOND).toInt())
        }
        updateStatusNotification()
    }

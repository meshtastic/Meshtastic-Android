...
// rendered offscreen with Compose Desktop's ImageComposeScene over one sample mesh, the real MapLibre map captured
// through maplibre-compose's MapSnapshotter, and each shot framed in a phone bezel under a captioned banner at
// 1242x2484. It has no tests, so `./gradlew test` / `allTests` never touch it; the one command is
//
//     ./gradlew :marketing-screenshots:updateMarketingScreenshots
//
// which writes straight into fastlane/metadata/android/<locale>/images/phoneScreenshots/. Only en-US is generated
// (-PmarketingLocales, default "en-US") and committed: everything under fastlane/ is read straight from git by
// F-Droid and IzzyOnDroid, and 42 more locales are ~55 MB per regeneration, so those are a CI + `fastlane supply`
// job for later. The map needs a Vulkan loader the JVM can find: on a stock Ubuntu nothing, on a Nix host, whose
// shell replaces LD_LIBRARY_PATH with its own, pass -PmarketingLibraryPath=/usr/lib/x86_64-linux-gnu.
plugins {
...
tasks.register<JavaExec>("updateMarketingScreenshots") {
    description =
        "Renders the store-listing screenshots into fastlane/metadata/android/<locale>/images/phoneScreenshots/."
    ...
    argumentProviders.add(CommandLineArgumentProvider { listOf(output.asFile.absolutePath, locales.get()) })

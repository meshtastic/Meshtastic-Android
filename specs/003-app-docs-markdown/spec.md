...
- **FR-009**: On supported Android 14+ devices running the `google` flavor, Help & Documentation MUST expose free-text Q&A powered by Gemini Nano running on-device through AICore/Google AI Edge or the ML Kit GenAI Prompt API.
...
- **FR-024**: Help & Documentation MUST be addressable by a deep link that resolves to the Settings graph. The external contract MUST accept `meshtastic://meshtastic/settings/helpDocs` and route to the typed docs entry point. Internal routing MAY normalize the slug to `help-docs` as long as the external contract remains stable.
...
- Q: What deep link should external callers use? → A: `meshtastic://meshtastic/settings/helpDocs`, with an internal alias acceptable as long as the contract remains backward compatible.

...
| `bandwidth` | Integer | Numeric input outside `LORA_24`; region/target-aware dropdown in `LORA_24` | Visible only when `use_preset = false`; `0` requires an explicit custom selection, while unsupported stored `LORA_24` values remain visible; both block Save until replaced |

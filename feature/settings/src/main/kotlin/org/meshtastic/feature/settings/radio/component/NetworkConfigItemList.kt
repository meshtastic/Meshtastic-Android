...
fun NetworkConfigScreen(viewModel: RadioConfigViewModel = hiltViewModel(), onBack: () -> Unit) {
    ...
    val formState = rememberConfigState(initialValue = networkConfig, adapter = Config.NetworkConfig.ADAPTER)
            ...
            item {
                TitledCard(title = stringResource(Res.string.udp_config)) {
                    SwitchPreference(
                        ...
                        onCheckedChange = {
                            formState.value =
                                formState.value.copy(enabled_protocols = if (it) 1 else 0)
                        },
                        containerColor = CardDefaults.cardColors().containerColor,
                    )
                }
        ...
        item {
            TitledCard(title = stringResource(Res.string.advanced)) {
                ...
                DropDownPreference(
                    ...
                    enabled = state.connected,
                    items =
                    Config.NetworkConfig.AddressMode.entries
                        .filter { it != Config.NetworkConfig.AddressMode.UNRECOGNIZED }
                        .map { it to it.name },
                ...
                EditIPv4Preference(
                    ...
                    value = formState.value.ipv4_config?.ip ?: 0,
                    enabled = state.connected && formState.value.address_mode == Config.NetworkConfig.AddressMode.STATIC,
                    keyboardActions = KeyboardActions(onDone = { focusManager.clearFocus() }),
                    onValueChanged = {
                        val ipv4 = (formState.value.ipv4_config ?: Config.NetworkConfig.IpV4Config()).copy(ip = it)
                        formState.value = formState.value.copy(ipv4_config = ipv4)
                    },
                )
                HorizontalDivider()
                EditIPv4Preference(
                    title = stringResource(Res.string.gateway),
                    value = formState.value.ipv4_config?.gateway ?: 0,
                    enabled = state.connected && formState.value.address_mode == Config.NetworkConfig.AddressMode.STATIC,
                    keyboardActions = KeyboardActions(onDone = { focusManager.clearFocus() }),
                    onValueChanged = {
                        val ipv4 = (formState.value.ipv4_config ?: Config.NetworkConfig.IpV4Config()).copy(gateway = it)
                        formState.value = formState.value.copy(ipv4_config = ipv4)
                    },
                )
                HorizontalDivider()
                EditIPv4Preference(
                    ...
                    value = formState.value.ipv4_config?.subnet ?: 0,
                    enabled = state.connected && formState.value.address_mode == Config.NetworkConfig.AddressMode.STATIC,
                    keyboardActions = KeyboardActions(onDone = { focusManager.clearFocus() }),
                    onValueChanged = {
                        val ipv4 = (formState.value.ipv4_config ?: Config.NetworkConfig.IpV4Config()).copy(subnet = it)
                        formState.value = formState.value.copy(ipv4_config = ipv4)
                    },
                )
                HorizontalDivider()
                EditIPv4Preference(
                    title = "DNS",
                    value = formState.value.ipv4_config?.dns ?: 0,
                    enabled = state.connected && formState.value.address_mode == Config.NetworkConfig.AddressMode.STATIC,
                    keyboardActions = KeyboardActions(onDone = { focusManager.clearFocus() }),
                    onValueChanged = {
                        val ipv4 = (formState.value.ipv4_config ?: Config.NetworkConfig.IpV4Config()).copy(dns = it)
                        formState.value = formState.value.copy(ipv4_config = ipv4)
                    },
                )
            }

...
fun DetectionSensorConfigScreen(viewModel: RadioConfigViewModel, onBack: () -> Unit) {
    ...
    val detectionSensorConfig = state.moduleConfig.detection_sensor ?: ModuleConfig.DetectionSensorConfig.Builder().build()
        ...
        item {
            TitledCard(title = stringResource(Res.string.detection_sensor_config)) {
                ...
                DropDownPreference(
                    ...
                    onItemSelected = { formState.value = formState.value.newBuilder().also { wb -> wb.minimum_broadcast_secs = it.toInt() }.build() },
                )
                ...
                DropDownPreference(
                    ...
                    onItemSelected = { formState.value = formState.value.newBuilder().also { wb -> wb.state_broadcast_secs = it.toInt() }.build() },
                )
                HorizontalDivider()
                SwitchPreference(
                ...
                EditTextPreference(
                    ...
                    onValueChanged = { formState.value = formState.value.newBuilder().also { wb -> wb.name = it }.build() },
                )
                HorizontalDivider()
                ...
                DropDownPreference(
                    ...
                    enabled = state.connected,
                    onItemSelected = { formState.value = formState.value.newBuilder().also { wb -> wb.monitor_pin = it }.build() },
                )
                HorizontalDivider()
                DropDownPreference(
                    ...
                    onItemSelected = { formState.value = formState.value.newBuilder().also { wb -> wb.detection_trigger_type = it }.build() },
                )
                HorizontalDivider()
                SwitchPreference(
                    ...
                    enabled = state.connected,
                    onCheckedChange = { formState.value = formState.value.newBuilder().also { wb -> wb.use_pullup = it }.build() },
                    containerColor = CardDefaults.cardColors().containerColor,
                )
                HorizontalDivider()
            }

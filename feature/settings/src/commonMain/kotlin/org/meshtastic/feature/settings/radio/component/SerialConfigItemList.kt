        ...
        item {
            TitledCard(title = stringResource(Res.string.serial_config)) {
                ...
                SwitchPreference(
                    ...
                    enabled = state.connected,
                    onCheckedChange = { formState.value = formState.value.newBuilder().also { wb -> wb.echo = it }.build() },
                    containerColor = CardDefaults.cardColors().containerColor,
                )
                HorizontalDivider()
                EditTextPreference(
                    ...
                    onValueChanged = { formState.value = formState.value.newBuilder().also { wb -> wb.rxd = it }.build() },
                )
                HorizontalDivider()
                EditTextPreference(
                    ...
                    enabled = state.connected,
                    keyboardActions = KeyboardActions(onDone = { focusManager.clearFocus() }),
                    onValueChanged = { formState.value = formState.value.newBuilder().also { wb -> wb.txd = it }.build() },
                )
                HorizontalDivider()
                DropDownPreference(
                    ...
                    onItemSelected = { formState.value = formState.value.newBuilder().also { wb -> wb.baud = it }.build() },
                )
                HorizontalDivider()
                EditTextPreference(
                    ...
                    enabled = state.connected,
                    keyboardActions = KeyboardActions(onDone = { focusManager.clearFocus() }),
                    onValueChanged = { formState.value = formState.value.newBuilder().also { wb -> wb.timeout = it }.build() },
                )
                HorizontalDivider()
                DropDownPreference(
                ...
                SwitchPreference(
                    ...
                    enabled = state.connected,
                    onCheckedChange = { formState.value = formState.value.newBuilder().also { wb -> wb.override_console_serial_port = it }.build() },
                    containerColor = CardDefaults.cardColors().containerColor,
                )
            }
        }

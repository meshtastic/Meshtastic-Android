    ...
    if (selectedRole != formState.value.role) {
        if (selectedRole in infrastructureRoles) {
            RouterRoleConfirmationDialog(
                ...
                onConfirm = { formState.value = formState.value.newBuilder().also { wb -> wb.role = selectedRole }.build() },
            )
        } else {
        ...
        item {
            TitledCard(title = stringResource(Res.string.options)) {
                ...
                DropDownPreference(
                    ...
                    onItemSelected = { formState.value = formState.value.newBuilder().also { wb -> wb.rebroadcast_mode = it }.build() },
                ...
                DropDownPreference(
                    ...
                    onItemSelected = { formState.value = formState.value.newBuilder().also { wb -> wb.node_info_broadcast_secs = it.toInt() }.build() },
                )
            }
        }
        ...
        item {
            TitledCard(title = stringResource(Res.string.hardware)) {
                SwitchPreference(
                    ...
                    enabled = state.connected,
                    onCheckedChange = { formState.value = formState.value.newBuilder().also { wb -> wb.double_tap_as_button_press = it }.build() },
                ...
                SwitchPreference(
                    ...
                    enabled = state.connected,
                    onCheckedChange = { formState.value = formState.value.newBuilder().also { wb -> wb.disable_triple_click = !it }.build() },
                    containerColor = CardDefaults.cardColors().containerColor,
                )
                ...
                SwitchPreference(
                    ...
                    enabled = state.connected,
                    onCheckedChange = { formState.value = formState.value.newBuilder().also { wb -> wb.led_heartbeat_disabled = !it }.build() },
                    containerColor = CardDefaults.cardColors().containerColor,
                )
            }
        }
        item {
            TitledCard(title = stringResource(Res.string.time_zone)) {
                ...
                EditTextPreference(
                    ...
                    onValueChanged = { formState.value = formState.value.newBuilder().also { wb -> wb.tzdef = it }.build() },
                    trailingIcon = {
                        IconButton(onClick = { formState.value = formState.value.newBuilder().also { wb -> wb.tzdef = "" }.build() }) {
                            Icon(
                ...
                TextButton(
                    ...
                    onClick = { formState.value = formState.value.newBuilder().also { wb -> wb.tzdef = appTzPosixString }.build() },
                ) {
                    Icon(
        ...
        item {
            TitledCard(title = stringResource(Res.string.gpio)) {
                EditTextPreference(
                    ...
                    enabled = state.connected,
                    keyboardActions = KeyboardActions(onDone = { focusManager.clearFocus() }),
                    onValueChanged = { formState.value = formState.value.newBuilder().also { wb -> wb.button_gpio = it }.build() },
                )
                ...
                EditTextPreference(
                    ...
                    enabled = state.connected,
                    keyboardActions = KeyboardActions(onDone = { focusManager.clearFocus() }),
                    onValueChanged = { formState.value = formState.value.newBuilder().also { wb -> wb.buzzer_gpio = it }.build() },
                )
            }
        }

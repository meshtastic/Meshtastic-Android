...
fun AmbientLightingConfigScreen(viewModel: RadioConfigViewModel, onBack: () -> Unit) {
    ...
    val ambientLightingConfig = state.moduleConfig.ambient_lighting ?: ModuleConfig.AmbientLightingConfig.Builder().build()
        ...
        item {
            TitledCard(title = stringResource(Res.string.ambient_lighting_config)) {
                SwitchPreference(
                    ...
                    enabled = state.connected,
                    onCheckedChange = { formState.value = formState.value.newBuilder().also { wb -> wb.led_state = it }.build() },

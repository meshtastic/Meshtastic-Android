...
class UserNameFieldsTest {
    ...
    fun `the ham long name field only exists while licensed`() = runComposeUiTest {
        showNameFields(User.Builder().also { wb ->wb.long_name = "Attic Heltec"; wb.short_name = "ATTC"}.build(), hamMode = false)
    ...
    fun `the call sign field shows only the call sign half of a composed name`() = runComposeUiTest {
        showNameFields(User.Builder().also { wb ->wb.long_name = "KD2ABC//Attic Heltec"; wb.short_name = "ABC"}.build(), hamMode = true)
    ...
    fun `editing the call sign keeps the long name half`() = runComposeUiTest {
        val user = showNameFields(User.Builder().also { wb ->wb.long_name = "KD2ABC//Attic Heltec"; wb.short_name = "ABC"}.build(), hamMode = true)
    ...
    fun `editing the long name keeps the call sign`() = runComposeUiTest {
        val user = showNameFields(User.Builder().also { wb ->wb.long_name = "KD2ABC"; wb.short_name = "ABC"}.build(), hamMode = true)
    ...
    fun `the widest pair the proto can carry composes whole`() = runComposeUiTest {
        val user = showNameFields(User.Builder().also { wb ->wb.long_name = "KD2ABCD"; wb.short_name = "ABC"}.build(), hamMode = true)

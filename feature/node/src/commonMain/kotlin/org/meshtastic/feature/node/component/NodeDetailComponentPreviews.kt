...
fun PowerMetricsPreview() {
    val node =
        previewData.mickeyMouse.copy(
            powerMetrics =
            org.meshtastic.proto.PowerMetrics.Builder().also { wb ->
...
fun PowerMetricsPartialPreview() {
    ...
    val node =
        previewData.mickeyMouse.copy(
            powerMetrics = org.meshtastic.proto.PowerMetrics.Builder().also { wb ->wb.ch1_voltage = 4.19f; wb.ch1_current = 128.4f}.build(),
        )
    AppTheme { Surface { PowerMetrics(node = node) } }
}
...
fun PowerMetricsNoCurrentPreview() {
    ...
    val node =
        previewData.mickeyMouse.copy(
            powerMetrics = org.meshtastic.proto.PowerMetrics.Builder().also { wb ->wb.ch1_voltage = 4.19f; wb.ch2_voltage = 3.72f}.build(),
        )
    AppTheme { Surface { PowerMetrics(node = node) } }
}
...
fun AirQualityInfoCardsPreview() {
    val node =
        previewData.mickeyMouse.copy(
            airQualityMetrics =
            org.meshtastic.proto.AirQualityMetrics.Builder().also { wb ->
            wb.pm10_standard = 8
            wb.pm25_standard = 12

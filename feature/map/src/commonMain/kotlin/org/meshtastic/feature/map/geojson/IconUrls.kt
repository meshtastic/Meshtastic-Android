...
/**
 * The property an imported feature names its icon image in.
 *
 * Deliberately ours rather than borrowed: simplestyle-spec 1.1.0 defines `marker-symbol` (a fixed icon vocabulary),
 * `marker-color` and `marker-size`, and nothing for an arbitrary image URL. No later specification has blessed one
 * either — the modern answer is layer-level styling, which is what the renderer does with this. Declared once so the
 * KML converter that writes it and the renderer that reads it cannot drift.
 */
const val ICON_URL_PROPERTY: String = "icon-url"

 ...
 * @param latLng The clicked location
...
fun handleMapClick(
    ...
    Timber.tag("MapClickHandlers")
        .d("Map click at (%.5f, %.5f) -> %d features", latLng.latitude, latLng.longitude, features.size)
    ...
    if (f != null && f.hasProperty("point_count")) {
        ...
        val limit = kotlin.math.min(CLUSTER_LIST_FETCH_MAX.toLong(), pointCount.toLong())
        ...
        if (src != null) {
            ...
            if (members.isNotEmpty()) {
                ...
                if (geom is Point) {
                    ...
                    map.animateCamera(
                        ...
                        object : MapLibreMap.CancelableCallback {
                            override fun onFinish() {
                                ...
                                val clusterCenter = map.projection.toScreenLocation(LatLng(clusterLat, clusterLon))
                    ...
                    Timber.tag("MapClickHandlers")
                        .d("Centering on cluster at (%.5f, %.5f) with %d members", clusterLat, clusterLon, members.size)
                } else {
                    ...
                    if (pointCount > CLUSTER_RADIAL_MAX) {
                        ...
                        onExpandedClusterChange(ExpandedCluster(clusterCenter, members.take(CLUSTER_RADIAL_MAX)))
                    }
                }
            }
            return true
        ...
        when (kind) {
            "node" -> {
                ...
                if (geom is Point) {
                    ...
                    Timber.tag("MapClickHandlers").d("Centering on node %d at (%.5f, %.5f)", num, nodeLat, nodeLon)
                }
            }
            "waypoint" -> {
                val id = it.getNumberProperty("id")?.toInt() ?: -1
                // Open edit dialog for waypoint
                val waypoint = waypoints.values.find { pkt -> pkt.data.waypoint?.id == id }?.data?.waypoint
                ...
                if (geom is Point) {
                    ...
                    Timber.tag("MapClickHandlers").d("Centering on waypoint %d at (%.5f, %.5f)", id, wpLat, wpLon)
                }
            }
            else -> {}
        }

...
- **KML import**: `KmlToGeoJson` converts KML into the GeoJSON both renderers already draw, parsed with xmlutil so it is common code rather than Android's pull parser. Recognising a KMZ and unpacking the zip stays with the host, which is the part that is genuinely a file.
